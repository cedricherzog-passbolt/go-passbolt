...
// made it effectively untestable before Go 1.27: asserting on the backoff meant
// really sleeping. These tests run inside a testing/synctest bubble, where
// time.Sleep advances a virtual clock instantly, so the retry count and the delay
// between attempts are both observable in zero wall-clock time.
//
// The mock server must be httptest.NewTestServer: it serves over an in-memory
// network, and a server holding a real socket is not usable inside a bubble
// (goroutines blocked on a real network read are not durably blocked, so the
// bubble would deadlock rather than advance its clock).
