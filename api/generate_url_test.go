...
// generateURL the Client's own baseURL pointer: it clones, so repeated calls stay
// independent and the caller's URL is untouched. Without the clone, the second
// call would join onto the first call's path.
